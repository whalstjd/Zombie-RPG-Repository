...
public class MonsterAnimation : MonsterCharacterAnimation
{
    ...
    public void After_Animating_Damaged()
    {
        if (myMonster.curState == MonsterState.DIE) return;

        myMonster.StateChange(MonsterState.MOVE);
    }
    public void After_Animating_Attack()
    {
        if (myMonster.curState == MonsterState.DIE) return;

        myMonster.Attack();
        myMonster.StateChange(MonsterState.IDLE);
        myMonster.curAttackDelay = myMonster.monsterStat.AttackSpeed;
    }
